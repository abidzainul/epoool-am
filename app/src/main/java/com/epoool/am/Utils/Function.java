    ...
    public static void sendTokenDriver(Context context) {
        try {
            FirebaseMessaging.getInstance().getToken().addOnCompleteListener(task -> {
                ...
                ok.posting(context,
                        Constant.url + "update_token_fcm",
                        Function.paramAdderPost(params),
                        runnable,
                        false, null, null);
            });
//        String url = Uri.parse(Function.paramAdderGet(Constant.url + "update_token_fcm"))
//        .buildUpon()
//        .appendQueryParameter("token_fcm", FirebaseInstanceId.getInstance().getToken())
//        .build().toString();
//
//        volleyUtil.requestGet(context,url,runnable,false,null,null);
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
    ...
    static void sendToken(Context context, Task task) {
        if (!task.isSuccessful()) {
            task.getException().printStackTrace();
            return;
        }
    ...
    static void statusSendToken(OkHttpHelper okHttpHelper) {
